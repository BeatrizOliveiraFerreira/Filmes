...
SELECT Filmes.* 
FROM Filmes 
INNER JOIN FilmesGenero ON Filmes.IdFilme = FilmesGenero.IdFilme 
INNER JOIN Generos ON FilmesGenero.IdGenero = Generos.IdGenero 
WHERE Generos.NomeGenero = 'Ação';
SELECT Atores.* 
FROM Atores 
INNER JOIN ElencoFilme ON Atores.IdAtor = ElencoFilme.IdAtor 
INNER JOIN Filmes ON ElencoFilme.IdFilme = Filmes.IdFilme 
WHERE Filmes.Titulo = 'Titanic';
SELECT Filmes.* 
FROM Filmes 
INNER JOIN ElencoFilme ON Filmes.IdFilme = ElencoFilme.IdFilme 
INNER JOIN Atores ON ElencoFilme.IdAtor = Atores.IdAtor 
WHERE Atores.NomeAtor = 'Leonardo DiCaprio';
SELECT Atores.* 
FROM Atores 
INNER JOIN ElencoFilme ON Atores.IdAtor = ElencoFilme.IdAtor 
INNER JOIN Filmes ON ElencoFilme.IdFilme = Filmes.IdFilme 
WHERE YEAR(Filmes.DataLancamento) = 1994;

SELECT Filmes.* 
FROM Filmes 
INNER JOIN FilmesGenero ON Filmes.IdFilme = FilmesGenero.IdFilme 
INNER JOIN Generos ON FilmesGenero.IdGenero = Generos.IdGenero 
WHERE Generos.NomeGenero = 'Drama' AND YEAR(Filmes.DataLancamento) = 2008;
...
INNER JOIN ElencoFilme ON Atores.IdAtor = ElencoFilme.IdAtor 
INNER JOIN Filmes ON ElencoFilme.IdFilme = Filmes.IdFilme 
WHERE Filmes.Classificacao = 'PG-13';


SELECT Filmes.* 
FROM Filmes 
WHERE Filmes.Diretor = 'Christopher Nolan';

SELECT Atores.* 
FROM Atores 
INNER JOIN ElencoFilme ON Atores.IdAtor = ElencoFilme.IdAtor 
INNER JOIN Filmes ON ElencoFilme.IdFilme = Filmes.IdFilme 
WHERE Filmes.Classificacao = 'R' AND YEAR(Filmes.DataLancamento) = 1999;


SELECT DISTINCT Generos.NomeGenero 
FROM Generos 
INNER JOIN FilmesGenero ON Generos.IdGenero = FilmesGenero.IdGenero 
INNER JOIN Filmes ON FilmesGenero.IdFilme = Filmes.IdFilme 
INNER JOIN ElencoFilme ON Filmes.IdFilme = ElencoFilme.IdFilme 
INNER JOIN Atores ON ElencoFilme.IdAtor = Atores.IdAtor 
WHERE Atores.NomeAtor = 'Tom Hanks';
